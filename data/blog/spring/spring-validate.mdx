...
"/some-api" 의 api 는 매우 단순하여 에러를 금방 찾을 수 있지만, 실제로 사용하는 api 의 경우 필드가 많아 찾기 힘들수도 있다. 어떤 필드에서 오류가 발생했는지 찾는데만 상당한 시간을 소비해야한다.

어떻게하면 더 유용한 에러를 보여줄 수 있을까??
...
이 패키지는 Request 필드를 검증할 때 사용하는 validation 어노테이션을 제공한다. Request 필드의 오류를 반환할 것이 아니라면 위 의존성은 추가하지 않아도 된다.
...
예를 들어 BusinessLogicException 같은 커스텀 Exception 을 만들어 서비스 layer 에서 에러가 발생할 경우 에러를 던지게 하는 식으로 계층을 구분하여 만들 수 있다.
