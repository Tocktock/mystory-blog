...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `npm run check`                    | 92 files, 0 errors, 0 warnings, 0 hints                                                                                     | Verified                             |
...
Local evidence artifacts are under `.reports/`, including `.reports/publication-safety/report.json`, `.reports/static-links/report.json`, `.reports/keyboard/report.json`, `.reports/responsive/report.json`, and `.reports/responsive/screenshots/`. Older slice screenshots also remain under `output/playwright/i09-final-qa-20260704/` on this machine. Those paths are ignored build/report outputs, so the table above captures the important result summary for review. `npm run audit:web` now runs a fresh build plus release-safety checks before opening `astro preview`; older evidence used an explicit build-before-audit sequence.
...
| SC-016   | MUST_PASS  | in_this_slice          | Check, lint, build, E2E, Lighthouse, `npm run audit:axe`, `npm run audit:keyboard`, `npm run audit:publication-safety`, `npm run audit:responsive`, `npm run audit:static-links`, `npm run audit:links`, `npm run audit:web`, keyboard spot check, RSS/dark-mode/search accessibility regressions, screenshots. | None for local automated acceptance.                   |
...
| Rollout              | Local evidence supports a review-ready PR; CI push triggers include the current default branch `master`. Production readiness is not claimed.                                                                | Human/external verification required      |
