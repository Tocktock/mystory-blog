...
| ------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------ |
| `npm run check`                 | 89 files, 0 errors, 0 warnings, 0 hints                                                                | Verified                             |
...
| `npm run audit:web`             | Fresh build plus preview audit passed Lighthouse, axe, responsive, and same-origin link crawl          | Verified                             |
...
Local evidence artifacts are under `.reports/`, including `.reports/responsive/report.json` and `.reports/responsive/screenshots/`. Older slice screenshots also remain under `output/playwright/i09-final-qa-20260704/` on this machine. Those paths are ignored build/report outputs, so the table above captures the important result summary for review. `npm run audit:web` now runs a fresh build before opening `astro preview`; older evidence used an explicit build-before-audit sequence.
...
| SC-016   | MUST_PASS  | in_this_slice          | Check, lint, build, E2E, Lighthouse, `npm run audit:axe`, `npm run audit:responsive`, `npm run audit:links`, `npm run audit:web`, static local link check, keyboard spot check, RSS/dark-mode/search accessibility regressions, screenshots. | None for local automated acceptance.                   |
...
| I-09 Final QA and release evidence            | human_required         | Final local checks, reproducible screenshot matrix via `npm run audit:responsive`, `npm run audit:web`, local link check, keyboard spot check, RSS/dark-mode/search accessibility regressions, CI default-branch trigger alignment, release packet, QA appendix, review-notes template, slice contract register, and local PR draft. | Filled human review notes, slice contract sign-off, plus production/deployment verification if release is requested. |
